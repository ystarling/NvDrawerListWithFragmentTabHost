...
public class TransactionFragmentTabHostSwipe extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		tabHost.setup();
		tabHost.addTab(tabHost.newTabSpec("Tab1").setIndicator("SRC").setContent(mFactory));
		tabHost.addTab(tabHost.newTabSpec("Tab2").setIndicator("DEST").setContent(mFactory));
		tabHost.addTab(tabHost.newTabSpec("Tab3").setIndicator("TX").setContent(mFactory));
	...
	private class TransactionInnerPagerAdapter extends FragmentPagerAdapter {
		...
		public Fragment getItem(int position) {
			if (position == 0) {
				return new SourceFragmentTab();
			} else if (position == 1) {
				return new DestinationFragmentTab();
			} else if (position == 2) {
				return new TransferFragmentTab();
			}
			return null;
		}
